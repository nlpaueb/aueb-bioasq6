...
def produce_pos_neg_pairs(data, docset, idf_dict, term2ind, model_params, query_preprocessing, doc_preprocessing, keep_up_to_year=sys.maxsize):
	...
	for q in data['queries']:
		...
		for pos_doc in rel_ret_set:
			...
			# Choose negative document published up to "keep_up_to_year"
			found = False
			tries = 0
			if non_rel_set:
				while not found and tries < len(non_rel_set):
					...
					try:
						...
						continue
					found = (pub_year <= keep_up_to_year)
					tries += 1
			if not found:
				continue
...
def produce_reranking_inputs(data, docset, idf_dict, term2ind, model_params, query_preprocessing, doc_preprocessing, keep_up_to_year=sys.maxsize):
	...
	for q in data['queries']:
		...
		for doc in q['retrieved_documents']:
			...
			# Discard document if published after "keep_up_to_year"
			try:
...
def bioasq_eval_custom(q_rels_file, metrics, path):
	...
	eval_res = subprocess.Popen(
		['java', '-Xmx10G', '-cp', '$CLASSPATH:../../../bioasq_eval/flat/BioASQEvaluation/dist/BioASQEvaluation.jar', 
		'evaluation.EvaluatorTask1b', '-phaseA', '-e', '5', q_rels_file, path],
	...
	results['f1'] = float(splitted_eval_res[7])
...
def rerank(queries_to_rerank, filename, scoring_model, qrels_file, model_params, metrics, retr_dir, doc_id_prefix, keep_topk):
		...
		for q in tqdm(queries_to_rerank, desc='queries'):
			...
			res_dict['questions'].append({'id': q['id'], 'documents': [doc_id_prefix + d[0] for d in sorted_retr_scores][:keep_topk]})

		path_bioasq = write_bioasq_results_dict(res_dict, retr_dir, filename)
		
		bioasq_eval_res = bioasq_eval_custom(qrels_file, metrics, path_bioasq)
		
		return bioasq_eval_res

def pacrr_train(train_pairs, dev_pairs, queries_to_rerank, term2ind, config, model_params, metrics, retr_dir, doc_id_prefix='', keep_topk=None):
	...
	class Evaluate(keras.callbacks.Callback):
		def on_epoch_end(self, epoch, logs={}):
			...
			dev_rerank_results = rerank(queries_to_rerank, 'dev_ranking_epoch{0}'.format(epoch), scoring_model, config['QRELS_DEV'], model_params, metrics, retr_dir, doc_id_prefix, keep_topk)
			...
			log('|'.join(list(map(str, [epoch, dev_rerank_results['map'], dev_rerank_results['gmap'], dev_rerank_results['f1'], logs['val_acc'], logs['val_loss'], logs['acc'], logs['loss']]))), retr_dir, 'log.txt')
	...
	log('epoch|dev_map|dev_gmap|dev_F1|dev_acc|dev_loss|train_acc|train_loss', retr_dir, 'log.txt')
	log('epoch|test_map|test_gmap|test_F1', retr_dir, 'test_log.txt')
